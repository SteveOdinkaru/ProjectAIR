...
const char* mqtt_server = ""; //IP Address of your mqtt server
const char* HostName = "edgex-mqtt-broker";
const char* topic_PHOTO = "edgexevents";
const char* topic_CONFIG = "JSONConfig";
const char* topic_UP = "PICTURE";
const char* mqttUser = "mqtt_admin";
const char* mqttPassword = "mqtt_admin";
...
void callback(String topic, byte* message, unsigned int length) {
  ...
  if (topic == topic_CONFIG) {//make sure the topic is the same with the one configured
...
//Reconnect function in case WiFi fails to connect
void reconnect() {
...
void loop() {
  ...
  client.loop();
  delay(43200); //Entire program set to run every twice a day i.e pictues are sent every 12 hours
}
